...
def searchDb():
    ...
    cursor = conn.cursor()
    cursor.execute(f'''SELECT * FROM ProgramFin
                    WHERE dog=?''', (d,))
...
def updateData():
    ...
    conn = sqlite3.connect('sqlite_python.db')
    cursor = conn.cursor()
    cursor.execute(f'''UPDATE ProgramFin SET idArt='{s1}', client1='{s2}', ful_name='{s3}', boss='{s4}', name='{s5}', 
...
def selected1(event):
    ...
    selection1 = str(clientchoosen.get()).replace('{','').replace('}','')
...
# Combobox creation
n2 = tk.StringVar()
clientchoosen = ttk.Combobox(frame1, width=30, textvariable=n2)

# Adding combobox drop down list
clientchoosen['values'] = searchscrollDb()
clientchoosen.grid(column=0, row=7)
clientchoosen.current()
clientchoosen.bind("<<ComboboxSelected>>", selected1)


def selected2(event):
    # получаем выделенный элемент
    selection2 = str(typychoosen.get())
    txt1.insert(0, selection2)
    typychoosen.set('')


n3 = tk.StringVar()
typychoosen = ttk.Combobox(frame1, width=30, textvariable=n3)
typychoosen['values'] = ["Договор поставки", "Договор аренды", "Договор работ"]
typychoosen.grid(column=0, row=5)
typychoosen.current()
typychoosen.bind("<<ComboboxSelected>>", selected2)
